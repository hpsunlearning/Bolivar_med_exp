...
for (ss in seq_along(fts)){
    ...
    df_row = df_row %>% mutate(Sign = case_when(LR <= -2 ~ "--",
                                                LR < 0 ~ "-",
                                                LR == 0 ~ "",
                                                LR < 2 ~ "+",
                                                LR >= 2 ~ "++"))
...
for (ss in seq_along(fts)){
    ...
    if(YY == 2015){
      ...
      df_row_2015 = data.frame(LR = lr_b_grp_2015[AR_g_2015[which(AR_g_2015[ES_cutoff_2015]==TRUE & !str_detect(AR_g_2015$otu.names, pattern = "Mitochondria|Chloroplast")), "otu.names"]])
    ...
    p1 <- draw(p, heatmap_legend_side = "left", annotation_legend_side = "left")
    pdf(file = paste0("output/figures/Ancom_heatmap_sig_log_rel_ab_by_grp2_", BB, "_", YY, ".pdf"), width = ComplexHeatmap:::width(p1) %>% as.numeric()/25.4, height = ComplexHeatmap:::height(p1) %>% as.numeric()/25.4, useDingbats = F)
    draw(p1)
    dev.off()
...
for (ss in seq_along(fts)){
    ...
    p1 <- draw(p, heatmap_legend_side = "left", annotation_legend_side = "left")
    pdf(file = paste0("output/figures/Ancom_heatmap_sig_log_rel_ab_year_", BB, ".pdf"), width = ComplexHeatmap:::width(p1) %>% as.numeric()/25.4, height = ComplexHeatmap:::height(p1) %>% as.numeric()/25.4, useDingbats = F)
    draw(p1)
    dev.off()
...
for (ss in c(1,3,4,5)){
    ...
    p1 <- draw(p, heatmap_legend_side = "left", annotation_legend_side = "left")
    pdf(file = paste0("output/figures/Ancom_heatmap_sig_log_rel_ab_Blasto_", BB, ".pdf"), width = ComplexHeatmap:::width(p1) %>% as.numeric()/25.4, height = ComplexHeatmap:::height(p1) %>% as.numeric()/25.4, useDingbats = F)
    draw(p1)
    dev.off()
